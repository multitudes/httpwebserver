 ...
 */
bool NetUtils::listenSocket(int server_socket) {
  int backlog = 10;
...
void NetUtils::checkForIdleConnections() {
  ...
  for (std::map<int, time_t>::iterator it = Networker::lastActivityTime.begin();
    ...
    if (now - it->second >
      ...
      debug(
          "[Server] Detected idle connection %d for a keep-alive timeout of %d",
          it->first, Networker::serverConfs[serveridx].keepalive_timeout);
...
std::string NetUtils::getclient_ip(int clientfd) {
  debugcolor(RED, "Getting client IP address for client socket %d", clientfd);
  return Networker::remoteAddresses[clientfd];
}
...
void NetUtils::sendErrorResponse(int clientfd, int status_code,
  ...
  debugcolor(YELLOW, "Sending error response to client socket %d %s", clientfd,
             response.c_str());
 ...
 * As input I do not need to pass a null terminated string, it supports binary
 * data Good for debugging. A binary buffer can be also of type uint8_t in
 * output I will have the same but in hex like "DE AD BE EF" The function return
