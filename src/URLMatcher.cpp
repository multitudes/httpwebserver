...
namespace URLMatcher
{
  ...
  void validateRequest(HTTPConnxData &conn)
  {
    ...
    if (conn.data.request.empty() || !conn.data.headers_received)
    {
      ...
      {
        debuglog(RED, "URLMatcher: Header parsing failed for fd %d.", conn.client_fd);
        SimpleResponse::htmlErrorResponse(conn, 400); // Bad Request
        HTTPServer::update_poll_events(conn.client_fd, POLLOUT);
        // Consider setting state to closing/error state before returning
        return;
      }
      // Check again if headers are now complete after parsing
      if (!conn.data.headers_received)
      {
        debuglog(YELLOW, "URLMatcher: Incomplete headers for fd %d. Waiting.", conn.client_fd);
        return; // Need more data
      }
      debuglog(GREEN, "URLMatcher: Headers parsed for fd %d.", conn.client_fd);
    }
    // --- End Placeholder ---
