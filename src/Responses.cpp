...
string addSessionCookieHeaders(HTTPConnxData &conn) {
    if (conn.data.has_session && !conn.data.session_id.empty()) {
        ...
        if (conn.data.response_headers.find("Set-Cookie: sessionid=") == string::npos) {
...
void htmlErrorResponse(HTTPConnxData &conn, int statusCode)
{
    ...
    if (conn.config->error_pages.find(statusCode) != conn.config->error_pages.end())
    {
        std::map<int, std::string>::const_iterator it = conn.config->error_pages.find(statusCode);
 ...
 */
void createResponse(HTTPConnxData &conn, std::string contentType, std::string response, int statusCode)
{
    ...
    string header;
    addStandardHeaders(conn, header, statusCode, contentType, static_cast<int>(response.size()));
