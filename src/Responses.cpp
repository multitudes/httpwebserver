...
void createResponse(HTTPConnxData &connection, std::string contentType,
                    std::string response, int statusCode) {
  string header = "HTTP/1.1 ";
  header += Utils::to_string(statusCode) + " " +
  ...
  header +=
      "Content-Length: " + Utils::to_string(static_cast<int>(response.size())) +
      "\r\n";
  header += "\r\n";
...
void htmlErrorResponse(HTTPConnxData &connection, int statusCode) {
  ...
  if (connection.urlMatcherData.config->error_pages.find(statusCode) !=
      connection.urlMatcherData.config->error_pages.end()) {
    // Replace 'auto' with the explicit iterator type
    std::map<int, std::string>::const_iterator it =
        connection.urlMatcherData.config->error_pages.find(statusCode);
...
void generatedHTMLResponse(HTTPConnxData &connection, int statusCode) {
  ...
  createResponse(connection, connection.urlMatcherData.content_type, htmlCode,
                 statusCode);
}
...
void simpleStatusResponse(HTTPConnxData &connection, int statusCode) {
  ...
  createResponse(connection, connection.urlMatcherData.content_type, response,
                 statusCode);
}
...
void prepareFileResponse(HTTPConnxData &conn, long fileSize) {
  ...
  header +=
      "Content-Length: " + Utils::to_string(static_cast<long long>(fileSize)) +
      "\r\n";
